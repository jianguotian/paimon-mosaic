...
if [[ -n "$RC_NUMBER" && "$TAG" != "v${RELEASE_VERSION}-rc${RC_NUMBER}" ]]; then
...
validate_maven_artifacts() {
  ...
  local ci_sources_jar="$NATIVE_DIR/java-package/mosaic-${RELEASE_VERSION}-sources.jar"
  ...
  if [[ ! -f "$test_classes/org/apache/paimon/mosaic/MosaicNativeLoaderSmokeTest.class" ]]; then
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  echo "The local Maven rebuild will be signed and deployed; the CI JAR is a validation reference."
fi
